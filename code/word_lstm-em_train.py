...
class WordLSTM:
    ...
    def __init__(self,word_vect_dict=None,tokenizer_save_path="",embedding_size=300):
    ...
    def prepare_training_data(self,raw_text):
        ...
        for i in range(len(text_without_unk)):
            ...
            #if(text_without_unk[i] not in self.word_vect_dict):
                text[i] = '<unk>'
                num_unk = num_unk + 1
...
folder = "k2wd05U2E300chunkShuffledRaw"
model_save_path = '../saved_models/' + folder + '/{epoch:02d}-{val_loss:.2f}.hdf5'
weight_save_path = '../saved_models/'+ folder +'/weights'
log_path = '../saved_models/'+ folder +'/log.csv'
tokenizer_save_path = '../saved_models/'+ folder +'/'
...
word_lstm = WordLSTM(tokenizer_save_path=tokenizer_save_path)
...
history = word_lstm.train(text=training_data,validation_split=0.05,
                          num_epochs=40, batch_size=20, bptt=35, dim_hidden=512, learning_rate=0.001,
